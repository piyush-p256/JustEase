...
const LawyerCard = ({lawyer}) => {
    ...
    const {name, avgRating, totalRating, photo, specialization, totalClients, firm} = lawyer

  return (
    <div className='p-3 lg:p-5'>
      <div>
        <img src={photo} className='w-full h-[300px] w-[300px] rounded-lg' alt="" />
      </div>
    ...
    <div className='mt-[18px] lg:mt-5 flex items-center justify-between'>
        <div>
            <h3 className='text-[16px] leading-7 lg:text-[18px] lg:leading-[30px] font-semibold text-headingColor'>+{totalClients} clients</h3>
            <p className='text-[14px] leading-6 font-[400] text-textColor'>{firm}</p>
        </div>

        <Link to='/lawyers/1' className='w-[44px] h-[44px] rounded-full border border-solid border-[#181A1E] flex items-center justify-center group hover:bg-primaryColor hover:border-none'>
